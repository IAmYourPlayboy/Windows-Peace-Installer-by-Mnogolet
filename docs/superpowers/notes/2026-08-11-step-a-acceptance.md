...
**Перечисление идёт в потоке интерфейса.** Замерено 240–1152 мс, и на это время
окно не отвечает. Раздел 9 архитектуры требует другого: «любая работа дольше
сотни миллисекунд уходит в фон, показывает продвижение и допускает отмену».
Расхождение между планом и архитектурой, а не дефект реализации: план сознательно
сделал модель экрана синхронной. Решение — за автором; работа небольшая
и локальная, затрагивает только `DiskPickerViewModel` и разметку экрана.
